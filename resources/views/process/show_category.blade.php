...
<body class="bg-gray-900 text-gray-100">
            ...
            <div class="relative flex items-center justify-between h-12">
                ...
                <div class="flex-shrink-0">
                    <a href="{{ route('process_program', ['program' => $program->id]) }}"
                        class="text-white px-3 py-1 rounded-md text-sm sm:text-base font-medium bg-gray-700 hover:bg-gray-600 transition duration-200">
                        Atrás
                    </a>
                </div>
    ...
    <div class="max-w-full mx-auto mt-4 px-4 sm:px-6 lg:px-8">
        ...
        <span class="text-white text-lg sm:text-xl font-semibold">REGISTRO DE EVALUACIÓN</span>
        <!-- Nombre del Programa -->
                    ...
                    <div class="mt-2 flex flex-col sm:flex-row sm:space-x-4 space-y-2 sm:space-y-0">
                        ...
                        @if (isset($reports[$evaluation->id]))
                            <a href="{{ route('process_edit_report', ['program' => $program->id, 'category' => $category->id, 'evaluation' => $evaluation->id, 'report' => $reports[$evaluation->id]->id]) }}"
                                class="bg-yellow-900 hover:bg-yellow-700 text-white font-bold py-1 px-3 rounded inline-block">
                                Modificar Reporte
                            </a>
                        @else
                            <a href="{{ route('process_create_report', ['program' => $program->id, 'category' => $category->id, 'evaluation' => $evaluation->id]) }}"
                                class="bg-green-900 hover:bg-green-700 text-white font-bold py-1 px-3 rounded inline-block">
                                Realizar Reporte
                            </a>
                        @endif
                    </div>
