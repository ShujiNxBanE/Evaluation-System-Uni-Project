...
<body class="bg-gray-800 text-gray-100">
    <div class="container mx-auto px-4 py-6">
        ...
        <h1 class="text-2xl font-bold mb-6">Editar Programa</h1>

        <form action="{{ route('update_program', ['program' => $program->id]) }}" method="GET" class="bg-gray-900 p-6 rounded-lg shadow-lg">
            @csrf
            <div class="mb-4">
                <label for="name" class="block text-lg font-medium mb-2">Nombre del programa:</label>
                <input type="text" id="name" name="name" value="{{ $program->name }}" maxlength="255" class="w-full px-3 py-2 bg-gray-700 text-gray-300 border border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div class="mb-4">
                <label for="description" class="block text-lg font-medium mb-2">Descripción del programa:</label>
                <textarea id="description" name="description" cols="30" rows="4" maxlength="255" class="w-full px-3 py-2 bg-gray-700 text-gray-300 border border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $program->description }}</textarea>
            </div>
            <div class="mb-4">
                ...
                <select id="user_id" name="user_id" class="w-full px-3 py-2 bg-gray-700 text-gray-300 border border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
