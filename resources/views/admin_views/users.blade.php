    ...
    <div class="container mx-auto px-4 py-10">
        ...
        <ul class="space-y-4">
            @foreach ($users as $user)
                <li class="bg-white p-4 rounded-lg shadow-lg border border-gray-300">
                    ...
                    <div class="mt-4 flex space-x-4 text-sm">
                        <a href="{{ route('admin_edit_user', ['user' => $user->id]) }}" class="text-blue-600 hover:text-blue-500">Editar Usuario</a>
                        <form action="{{ route('admin_destroy_user', ['user' => $user->id]) }}" method="POST" onsubmit="confirmDeletion(event, '{{ $user->name }}')">
                            @method('DELETE')
                            @csrf
                            <button type="submit" class="text-red-600 hover:text-red-500">Eliminar Usuario</button>
                        </form>
                    </div>
                </li>
            @endforeach
